...
// 	{
// 		// $logos = \App\Logo::all();
// 		$sliders = \App\Slider::limit(3)->get();
...
public function index()
{
    ...
    $data = Cache::remember('index_data', $minutes, function () {
        return [
            ...
            'whyChooseUs' => WhyChooseUs::first(),
            'features' => Feature::orderBy('order', 'asc')->get(),
            'feeCategory' => FeeCategory::orderBy('created_at', 'asc')
                    ->with('feeList')
                    ->get(),
    ...
    return view('front.index', $data);
	    ...
	    $photoList = \App\PhotoList::latest()->limit(8)->get();
...
//         $minutes=10;;
//         $cacheKey = 'yoga_class_' . $slug;

//         $data = Cache::remember($cacheKey, $minutes, function () use ($slug) {
//             return [
...
// 		return view('front.class', compact('information','faqs'));
// 	}
