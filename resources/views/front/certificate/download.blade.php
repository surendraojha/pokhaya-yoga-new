                ...
                <div class="cirtificate-border-one text-center">
                    <div class="cirtificate-border-two">
                       <div class="cirtificate-heading" style="">{{ __('frontstaticword.CertificateofCompletion') }}</div>
                        ...
                        @endphp
                       <p class="cirtificate-detail" style="font-size:18px">{{ __('frontstaticword.Thisistocertifythat') }}
                        <b>&nbsp;{{ Auth::User()['fname'] }}&nbsp;{{ Auth::User()['lname'] }}</b> {{ __('frontstaticword.successfullycompleted') }} <b>{{ $course['title'] }}</b>
                         <i>{{ $course['duration'] }}</i> {{ __('frontstaticword.onlinecourseon') }} <br>
                        @if(Auth::User()->role == "admin")
                        <span style="font-size:18px">{{ date('jS F Y', strtotime($mytime)) }}</span>
                        @else
                        <span style="font-size:18px">{{ date('jS F Y', strtotime($course['created_at'])) }}</span>
                        @endif
                      </p>
                       ...
                       <br>
                       <div>
                        <img src="{{ asset('images/user_img/'.$course->user['signature']) }}" style="height: 50px;">
                  </div>
