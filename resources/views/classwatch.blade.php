...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body class="player-course-chapter">
		...
		<ul id="courseplaylist" class="display-none">
			<li data-source="courseplaycontent{{ $class->id }}" data-playlist-name="{{ $class->coursechapters->chapter_name }}"
                data-thumbnail-path="{{ url('images/course/'.$class->courses->title) }}">
				...
				<li
					...
					@endphp
						...
						<li @if($skipad->ad_video !="no")
							...
							@else
							data-source="{{ $skipad->ad_url }}" @endif data-time-start="{{ $skipad->time }}" data-time-to-hold-ads="{{ $skipad->ad_hold }}"
                             data-thumbnail-source="{{asset('images/course/'.$class->courses->preview_image)}}"
                             data-link="{{ $skipad->ad_target }}" data-target="_blank"></li>
