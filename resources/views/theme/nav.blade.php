    ...
    <div class="container">
        ...
        <div class="navigation fullscreen-search-block">
            ...
            <div class="logo">
                ...
                <a href="{{ url('/') }}"><img src="{{ asset('images/logo/'.$setting->logo) }}" class="img-fluid"
                        alt="logo"></a>
                @else()
                <a href="{{ url('/') }}"><b>
                ...
                <div id="notification_li">
                    ...
                    <div id="notificationContainerr">
                        ...
                        <div id="notificationFooter"><a
                                href="{{route('deleteNotification')}}">{{ __('frontstaticword.ClearAll') }}</a></div>
                    </div>
                </div>
            </div>
            @endauth
            ...
            <div id="mySidenav" class="sidenav">
                ...
                <div class="login-block">
                    <a href="{{ route('register') }}" class="btn btn-primary"
                        title="register">{{ __('Register Free') }}</a>
                    <a href="{{ route('login') }}" class="btn btn-secondary"
                        title="login">{{ __('frontstaticword.Login') }}</a>
                    <a href="{{ route('login') }}" class="btn btn-secondary">
                        {{ __('frontstaticword.Login Student') }}
                    </a>
                                ...
                                <h4 class="panel-title">
                                    <a role="button" data-toggle="collapse" data-parent="#accordion"
                                        ...
                                        <i class="fa {{ $cate->icon }} rgt-10"></i> <label class="prime-cat"
                                            data-url="{{ route('category.page',$cate->id) }}">{{ str_limit($cate->title, $limit = 20, $end = '..') }}</label>
                                    </a>
                                </h4>
                            </div>
                                        ...
                                        <div class="panel-heading" role="tab" id="headingeleven">
                                            <h4 class="panel-title">
                                                <a class="collapsed" role="button" data-toggle="collapse"
                                                    data-parent="#accordion" href="#collapseeleven{{ $sub->id }}"
                                                    aria-expanded="false" aria-controls="collapseeleven">
                                                    <i class="fa {{ $sub->icon }} rgt-10"></i> <label class="sub-cate"
                                                        data-url="{{ route('subcategory.page',$sub->id) }}">{{ str_limit($sub->title, $limit = 15, $end = '..') }}</label>

                                                </a>
                                            </h4>
                                        </div>

                                        <div id="collapseeleven{{ $sub->id }}" class="panel-collapse collapse"
                                            role="tabpanel" aria-labelledby="headingeleven">
                                            @foreach($sub->childcategory as $child)
                                            <div class="panel-body sub-cat">
                                                <i class="fa {{ $child->icon }} rgt-10"></i> <label class="child-cate"
                                                    data-url="{{ route('childcategory.page',$child->id) }}">{{ $child->title }}
                                                </label>
                                            </div>
                                            @endforeach
                                        </div>
                ...
                <div class="row">
                    <div class="col-lg-6 col-md-4 col-sm-12">
                        <div class="logo">
                            ...
                            @if($setting->logo_type == 'L')
                            <a href="{{ url('/') }}"><img src="{{ asset('images/logo/'.$setting->logo) }}"
                    ...
                    <div class="col-lg-6 col-md-4 col-sm-12">
                        <div class="navigation">
                            <div id="cssmenu">
                                <ul>
                                    <li><a href="#" title="Categories"><i
                                                class="flaticon-grid"></i>{{ __('frontstaticword.Categories') }}</a>
                                        @php
                                        $categories = App\Categories::orderBy('position','ASC')->get();
                                        @endphp
                                        <ul>
                                            ...
                                            <li><a href="{{ route('category.page',$cate->id) }}"
                                                    title="{{ $cate->title }}"><i
                                                        class="fa {{ $cate->icon }} rgt-20"></i>{{ str_limit($cate->title, $limit = 25, $end = '..') }}<i
                                                        class="fa fa-chevron-right float-rgt"></i></a>
                                                <ul>
                                                    @foreach($cate->subcategory as $sub)
                                                    @if($sub->status ==1)
                                                    <li><a href="{{ route('subcategory.page',$sub->id) }}"
                                                            title="{{ $sub->title }}"><i
                                                                class="fa {{ $sub->icon }} rgt-20"></i>{{ str_limit($sub->title, $limit = 25, $end = '..') }}
                                                            <i class="fa fa-chevron-right float-rgt"></i></a>
                                                        <ul>
                                                            @foreach($sub->childcategory as $child)
                                                            @if($child->status ==1)
                                                            <li>
                                                                <a href="{{ route('childcategory.page',$child->id) }}"
                                                                    title="{{ $child->title }}"><i
                ...
                <div class="row">
                    <div class="col-lg-6 col-md-6">
                        <div class="learning-business">
                            ...
                            <a href="{{ route('login') }}" class="btn btn-link" data-toggle="tooltip"
                                data-placement="bottom"
                                title="Login/Register To Become an Instructor">{{ __('frontstaticword.BecomeAnInstructor') }}</a>
                            @endif
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6">
                        <div class="Login-btn">
                            ...
                            <a href="{{ route('register') }}" class="btn btn-primary"
                                title="register">{{ __('Register Free') }}</a>
                            <a href="{{ url('parentlogin') }}" class="btn btn-secondary"
                                title="login">{{ __('frontstaticword.Login') }}</a>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-2 col-md-2 col-6">
                            <div class="learning-business">
                                <a href="{{ route('mycourse.show') }}" class="btn btn-link"
                                    title="My Course">{{ __('frontstaticword.MyCourses') }}</a>
                            </div>
                        </div>
                                        ...
                                        <div id="notificationContainer">
                                            ...
                                            <div id="notificationsBody" class="notifications">
                                                <ul>
                                                    ...
                                                    $orders
                                                    =\App\Order::where('user_id',$user_id)->pluck('course_id')->all();


                                                    $read_notifications =
                                                    \Modules\Notification\Entities\ReadOtherNotification::where('user_id',$user_id)
                                                    ->pluck('other_notification_id')
                                                    ->all();


                                                    $other_notifications =
                                                    \Modules\Notification\Entities\OtherNotification::whereNotIn('id',$read_notifications)
                                                    ->whereIn('course_id',$orders)
                                                    ->get();

                                                    @endphp


                                                    @foreach ($other_notifications as $value)


                                                    @php
                                                    $course = \App\Course::find($value->course_id);

                                                    $course_image = $course->preview_image;
                                                    @endphp
                                                    ...
                                                    <li class="unread-notification">
                                                        ...
                                                        <a href="{{ url('read-other-notification/'.$value->id) }}">
                                                            <div class="notification-image">

                                                                @if($course_image)

                                                                <img src="{{ asset('images/course/'.$course_image) }}"
                                                                    alt="course" class="img-fluid">
                                                                @else
                                                                <img src="{{ Avatar::create($course->id)->toBase64() }}"
                                                                    alt="course" class="img-fluid">
                                                                @endif
                                                            </div>
                            ...
                            <div class="my-container">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right User-Dropdown U-open"
                                        ...
                                        <a target="_blank" href="{{ url('/admins') }}">
                                            <li><i
                                                    class="fa fa-dashboard"></i>{{ __('frontstaticword.AdminDashboard') }}
                                            </li>
                                        </a>
                                        @endif
                                        @if(Auth::User()->role == "instructor")
                                        ...
                                        <a target="_blank" href="{{ route('student.dashboard') }}">
                                            <li><i class="fa fa-dashboard"></i>{{ __('Student Dashboard') }}</li>
                                        </a>
                                        ...
                                        <a href="{{ route('purchase.show') }}">
                                            <li><i
                                                    class="fa fa-shopping-cart"></i>{{ __('frontstaticword.PurchaseHistory') }}
                                            </li>
                                        </a>
                                        <a href="{{route('profile.show',Auth::User()->id)}}">
                                            <li><i class="fa fa-user"></i>{{ __('frontstaticword.UserProfile') }}</li>
